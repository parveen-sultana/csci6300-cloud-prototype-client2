...
import { S1_URL_TECHNOLOGY_ADD, S1_URL_TECHNOLOGY_DELETE, S1_URL_TECHNOLOGY, BEARER_HEADER} from '../../constants'
...
class Server1 extends Component {
  ...
  handleSubmit = event => {
    ...
    console.log(S1_URL_TECHNOLOGY_ADD)
    event.preventDefault();
    axios.post(S1_URL_TECHNOLOGY_ADD, {
  ...
  handleDelete = event => {
    event.preventDefault();
    axios.delete(S1_URL_TECHNOLOGY_DELETE+'/'+this.state.name,
    {headers: BEARER_HEADER})
    .then((response) => {
      console.log("inside resp"+response)
  ...
  handleView = event => {
    ...
    axios.get(S1_URL_TECHNOLOGY,{headers: BEARER_HEADER}
    ).then((response)=> {
  ...
  render() {
    return(
      <div>
        ...
        <h1>Successfully authenticated via Server1.</h1>
        {this.state.message}
        <Form>
        <FormGroup>
